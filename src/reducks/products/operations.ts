    ...
    return async (
        ...
        productsRef
            ...
            .then(() => {
                ...
                const nextProducts = prevProducts.filter((product: { id: string }) => product.id !== id);
...
export const orderProduct = (productsInCart: any, amount: number) => {
    return async (
        ...
        if (soldOutProducts.length > 0) {
            const errorMessage = soldOutProducts.length > 0 ? soldOutProducts.join("と") : soldOutProducts[0];
...
export const saveProduct = (
    ...
    return async (dispatch: Dispatch<CallHistoryMethodAction<[string, unknown?]>>): Promise<void> => {
        const timeStamp = FirebaseTimeStamp.now();
